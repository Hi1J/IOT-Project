...
//int *reg_0001 = &NODE1_CONNECT_STATUS;
//int *reg_0002 = &NODE1_LIGHT;
//int *reg_0003 = &NODE1_PEOPLE;
//int *reg_0004 = &NODE2_CONNECT_STATUS;
//int *reg_0005 = &NODE2_LIGHT;
//int *reg_0006 = &NODE2_PEOPLE;


int reg_data(int number)
{
	switch(number)
	{
		case 1:
			return NODE1_CONNECT_STATUS;
		case 2:
			return NODE1_LIGHT;
		case 3:
			return NODE1_PEOPLE;
		case 4:
			return NODE2_CONNECT_STATUS;
		case 5:
			return NODE2_LIGHT;
		case 6:
			return NODE2_PEOPLE;
...
void MODBUS_H03(uint8_t *data)
{
	...
	number = number_count((data[2] << 8) | data[3]);
	number += 1;
	for(i=0;i<data[5];i++)
	{
		send_buf[3 + (i * 2)] = reg_data(number + i) >> 8;
		send_buf[4 + (i * 2)] = reg_data(number + i) & 255;
	}
...
void MODBUS_H06(uint8_t *data)
{
	...
	//reg_data(number) = (data[4] <<8) | data[5];
	for(i=0;i<8;i++)
	{
		rt_thread_mdelay(1);

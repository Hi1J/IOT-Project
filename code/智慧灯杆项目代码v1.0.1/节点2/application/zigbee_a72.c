...
void A72_HANDLE_DATA(void)
{
	...
	{
		if(rx_data_buf[1] == 0x7A)//ZIGBEE心跳包
		{
			uint8_t send_buf[2] = {0x02,0x7A};
			A72_SEND_DATA(_COORDINATOR,&A72_SEND_MODE_SHORT,send_buf,2);//回应心跳包
		}
		else if(rx_data_buf[1] == 0x8A)
		{
			if(rx_data_buf[2] == 0)
			{
				...
				Led_Status = 0;
				rt_kprintf("Control LED 0FF\n");
			}
			else if(rx_data_buf[2] == 1)
			{
				rcu_periph_clock_enable(RCU_TIMER2);
				//timer_enable(TIMER_2);
				Led_Status = 1;
				rt_kprintf("Control LED ON\n");
			}
		}
			
	
	}
	else if((rx_data_buf[0] == 0x01 || rx_data_buf[0] == 0x03) && rx_data_buf[1] == 0x78 && (rx_data_buf[2] == 0x12 || rx_data_buf[2] == 0x32))
	{
		Turn_next = 1;
	}

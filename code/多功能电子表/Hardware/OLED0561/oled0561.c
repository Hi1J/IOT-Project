...
//显示英文与数字8*16的ASCII码												//用法：两数相乘的开始输入的格数  宽一共128格
//取模大小为8*16，取模方式为“从左到右从上到下”“纵向8点下高位”				//注意：数字字母占8格 汉字占16格
...
void OLED_DISPLAY_16x16(u8 x, //显示汉字的页坐标（从0xB0到0xB7）
	...
	for(t=0;t<2;t++){
		...
		for(j=0;j<16;j++){ //整页内容填充
 			I2C_SAND_BYTE(OLED0561_ADD,DAT,GB_160[(w*32)+c]);
			c++;}x++; //页地址加1
	}
...
void OLED_DISPLAY_PIC1(void){ //显示全屏图片
	...
	for(m=0;m<8;m++){//
		...
		for(i=0;i<128;i++){//送入128次图片显示内容
			I2C_SAND_BYTE(OLED0561_ADD,DAT,PIC2[i+m*128]);}
	}
}

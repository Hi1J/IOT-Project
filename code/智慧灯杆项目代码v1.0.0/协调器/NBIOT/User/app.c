...
uint8_t NODE1_STATUS = 0;
uint16_t NODE1_LIGHT = 0;
int NODE1_PEOPLE = 0;
uint8_t RX_NODE1 = 0;

uint8_t NODE2_STATUS = 0;
...
void Turn_on_next(void *parameter)
{
	...
	while(1)
	{
		...
		buf[1] = NODE_Appear_dirction;
		switch(NODE_Appear_dirction)
		{
			case 0x12:
				A72_SEND_DATA(_NODE2,&A72_SEND_MODE_SHORT,buf,2);
				rt_kprintf("turn:1-->2");
				break;
			case 0x21:
				A72_SEND_DATA(_NODE1,&A72_SEND_MODE_SHORT,buf,2);
				rt_kprintf("turn:2-->1");
				break;
			default:
				break;

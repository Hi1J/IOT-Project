...
/*
Lora管脚的初始化
Tx -- USART3_RX -- PB10(串口3功能)
Rx -- USART3_TX -- PB11(串口3功能)
M0 -- SPI1_CS   -- PB0 (通用推挽输出模式)
M1 -- SPI1_SCK  -- PA5 (通用推挽输出模式)
AUX - SPI1_MISO -- PA6 (浮空输入模式)
*/
void Lora_Config(void)
{
	...
	USART3_Config(9600);//串口3初始化+接收中断初始化
	Time4_Config(72,20000);//定时器4初始化
...
void Lora_SendData(u8 *data,u16 len)
{
	...
	for(u8 i = 0;i<len;i++)
		buff[i+3] = data[i];
	UART_SendData(USART3,buff,len+3);
}
void Lora_SendCmd(u8 *data,u16 len)
{
	UART_SendData(USART3,data,len);
}
//设置Lora的模式
...
void Lora_CurrentPram(void)
{
	...
	do{
		...
		{
			lora_data.buff[RxCount++] = USART3->DR;
		}
...
//	memset(lora_data.buff,0,sizeof(lora_data.buff));
  ...
  lora_data.lora_workprm.head = CMD_STOREPRM;
	...
	delay_ms(LORA_SETPRAM_TIME);//延时等待Lora内部保存完成
	Lora_SetMode(LORA_MODE_PATTERN);//进入一般模式
	//开中断
	USART_ITConfig(USART3,USART_IT_RXNE,ENABLE);
	NVIC_EnableIRQ(USART3_IRQn);
